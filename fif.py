...
class Fif:
    def __init__(self, debug: bool = False):
        ...
        self.parse = {6: self._parse_labl, "set_labl": self._set_labl}
        self.commands = {
            1: self._putc,
            2: self._putn,
            4: self._getn,
    ...
    def _print_debug(self, length, message=None, include_stack=False):
        if self.debug:
            ...
            line_number = (
                f"line {str(self.index + 1).rjust(len(str(len(self.program))))}"
            )
            text = f"{line_number}: {str(length).ljust(3)}{str(message).ljust(6)}"
            if include_stack:
                print(
                    f"{text} - [{', '.join(str(x) for x in self.stacks[self.stack_key])}]"
                )
            else:
                print(text)

    def _putc(self, line: str, length: int, index: int):
    ...
    def _putn(self, line: str, length: int, index: int):
        pop_index = self.stack_indexes[self.stack_key]
    ...
    def _getn(self, line: str, length: int, index: int):
        self.stacks[self.stack_key].append(int(input()))
        self._print_debug(length, include_stack=True)

    def _gets(self, line: str, length: int, index: int):
        for ch in input():
            self.stacks[self.stack_key].append(ord(ch))
        self._print_debug(length, include_stack=True)

    def _labl(self, line: str, length: int, index: int):
        self._print_debug(length)
        return "labl"

    def __labl(self, line: str, length: int, index: int):
        # Essentally a noop
        pass

    def _jump(self, line: str, length: int, index: int):
        self._print_debug(length)
        return "do_jump"

    def _jz(self, line: str, length: int, index: int):
        check = self.stacks[self.stack_key][-1] == 0
        self._print_debug(
            length, message=f"jz {'T' if check else 'F'}", include_stack=True
        )
        return "do_jump" if check else "noop"

    def _jn(self, line: str, length: int, index: int):
        check = self.stacks[self.stack_key][-1] < 0
        self._print_debug(
            length, message=f"jn {'T' if check else 'F'}", include_stack=True
        )
        return "do_jump" if check else "noop"

    def _do_jump(self, line: str, length: int, index: int):
        self.index = self.labels[line]
        self._print_debug(self.index, "jump")

    def _exit(self, line: str, length: int, index: int):
        self._print_debug(length, "exit")
        quit()

    def _add(self, line: str, length: int, index: int) -> None:
        pop_index = self.stack_indexes[self.stack_key]
    ...
    def _sub(self, line: str, length: int, index: int) -> None:
        pop_index = self.stack_indexes[self.stack_key]
        right = self.stacks[self.stack_key].pop(pop_index)
        left = self.stacks[self.stack_key].pop(pop_index)
        self.stacks[self.stack_key].append(left - right)
    ...
    def _mul(self, line: str, length: int, index: int) -> None:
        pop_index = self.stack_indexes[self.stack_key]
        left = self.stacks[self.stack_key].pop(pop_index)
        right = self.stacks[self.stack_key].pop(pop_index)
        self.stacks[self.stack_key].append(left * right)
    ...
    def _div(self, line: str, length: int, index: int) -> None:
        pop_index = self.stack_indexes[self.stack_key]
        right = self.stacks[self.stack_key].pop(pop_index)
        left = self.stacks[self.stack_key].pop(pop_index)
    ...
    def _mod(self, line: str, length: int, index: int) -> None:
        pop_index = self.stack_indexes[self.stack_key]
        right = self.stacks[self.stack_key].pop(pop_index)
        left = self.stacks[self.stack_key].pop(pop_index)
        self.stacks[self.stack_key].append(math.floor(left % right))
    ...
    def _push(self, line: str, length: int, index: int) -> str:
        self._print_debug(length)
        return "push"

    def __push(self, line: str, length: int, index: int) -> None:
        self.stacks[self.stack_key].append(length)
        self._print_debug(length, "", True)

    def _dup(self, line: str, length: int, index: int) -> None:
        self.stacks[self.stack_key].append(self.stacks[self.stack_key][-1])
        self._print_debug(length, include_stack=True)

    def _swap(self, line: str, length: int, index: int) -> None:
        pop_index = self.stack_indexes[self.stack_key]
    ...
    def _pop(self, line: str, length: int, index: int) -> None:
        pop_index = self.stack_indexes[self.stack_key]
        self.stacks[self.stack_key].pop(pop_index)
        self._print_debug(length, include_stack=True)

    def _stck(self, line: str, length: int, index: int):
        pass

    def _noop(self, line: str, length: int, index: int) -> None:
        self._print_debug(length)

    def _parse_labl(self, line: str, length: int, index: int) -> str:
        return "set_labl"

    def _set_labl(self, line: str, length: int, index: int) -> None:
    ...
    def execute(self, program: list, program_name: str = None) -> None:
...
def execute(program: List[str], program_name: str = None, debug: bool = False) -> None:
    fif = Fif(debug)
